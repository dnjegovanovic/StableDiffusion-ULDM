...
import os                           # OS utilities (kept for compatibility)
import glob                         # file globbing for legacy paths (we'll also use pathlib)
...
from utils.diffusion_utils import load_latents  # your existing latent loader
...
class CelebDataset(Dataset):
    ...
    im_path : str | Path
        Root path containing:
          - CelebA-HQ-img/*.{png,jpg,jpeg}
          - optional celeba-caption/*.txt
          - optional CelebAMask-HQ-mask/*.png
    im_size : int
    ...
    def __init__(
        ...
        image_cond_cfg = condition_config.get("image_condition_config", {})  # nested config or {}
        self.mask_channels: int = int(image_cond_cfg.get(                    # number of classes/channels
            "image_condition_input_channels", 18
        ))
        self.mask_h: int = int(image_cond_cfg.get("image_condition_h", self.im_size))  # mask H
        self.mask_w: int = int(image_cond_cfg.get("image_condition_w", self.im_size))  # mask W
        ...
        self.image_transform = torchvision.transforms.Compose([
            torchvision.transforms.Resize(self.im_size),           # resize shortest side to im_size
            torchvision.transforms.CenterCrop(self.im_size),       # center crop to (im_size, im_size)
        ...
        if use_latents and (latent_path is not None):
            ...
            if hasattr(latent_maps, "__len__") and len(latent_maps) == len(self.images):
                ...
                print(f"[CelebDataset] Found {len(self.latent_maps)} latents (positional).")
            else:
                ...
                if isinstance(latent_maps, dict) and len(latent_maps) > 0:
                    ...
                    print(f"[CelebDataset] Found {len(self.latent_maps)} latents (dict).")
                else:
                    # Could not match latents to images
                    self.use_latents = False
                    self.latent_maps = None
                    print("[CelebDataset] Latents not aligned or not found; falling back to images.")

    def _load_image_lists(self, root: Path) -> Tuple[List[Path], List[List[str]], List[Path]]:
        """
        Collect image paths (CelebA-HQ-img/*.ext), optional caption lists, and optional mask paths.
        ...
        # Build candidate glob patterns under CelebA-HQ-img
        img_dir = root / "CelebA-HQ-img"                           # standard celeb-hq folder
        ...
        assert len(images) > 0, f"[CelebDataset] No images found in {img_dir} with {self.search_exts}"
        ...
        if "image" in self.condition_types:
            ...
            self.idx_to_cls_map = {idx: name for idx, name in enumerate(label_list)}  # map idx->name
            self.cls_to_idx_map = {name: idx for idx, name in enumerate(label_list)}  # map name->idx
            # Validate configured channel count matches labels length
            assert self.mask_channels == len(label_list), (
        ...
        for img_path in tqdm(images, desc="[CelebDataset] Scanning images"):
            ...
            masks.append(mask_path if mask_path is not None else Path())  # empty Path if None
        ...
        if "text" in self.condition_types:
            ...
            assert len(texts) == len(images), "[CelebDataset] Text conditioning misaligned with images."
            # Warn if some images have no captions
            missing_caps = sum(1 for caps in texts if len(caps) == 0)
            if missing_caps > 0:
                print(f"[CelebDataset] WARNING: {missing_caps} images have no captions.")

        if "image" in self.condition_types:
            # Ensure same number of mask entries as images
            assert len(masks) == len(images), "[CelebDataset] Image conditioning misaligned with images."
            # Count missing masks
            missing_masks = sum(1 for m in masks if (not m or not m.exists()))
            if missing_masks > 0:
                print(f"[CelebDataset] WARNING: {missing_masks} masks not found in CelebAMask-HQ-mask/.")
            # Informative counts
    ...
    def _load_mask(self, index: int) -> torch.Tensor:
        ...
        # Defensive checks
        assert isinstance(mask_path, Path), "[CelebDataset] Internal: mask path not a Path."
        ...
        one_hot = np.zeros((self.mask_h, self.mask_w, self.mask_channels), dtype=np.float32)
    ...
    def _resolve_latent(self, index: int) -> Any:
        ...
        raise KeyError(f"[CelebDataset] Could not match latent for image index {index}: {self.images[index]}")
