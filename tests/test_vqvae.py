...
def install_unetblocks_stub():
    ...
    class DownSamplingBlock(nn.Module):
        def __init__(self, in_channels, out_channels, time_emb_dim=None, down_sample=False,
                     num_heads=1, num_layers=1, use_attn=False, grp_norm_chanels=8):
            super().__init__()
    ...
    class BottleNeck(nn.Module):
        def __init__(self, in_channels, out_channels, time_emb_dim=None,
                     num_heads=1, num_layers=1, grp_norm_chanels=8):
            super().__init__()
            self.conv = nn.Conv2d(in_channels, out_channels, kernel_size=3, padding=1)
    ...
    class UpSamplingBlock(nn.Module):
        def __init__(self, in_channels, out_channels, time_emb_dim=None, up_sample=False,
                     num_heads=1, num_layers=1, use_attn=False, grp_norm_chanels=8):
            super().__init__()
...
def make_min_config():
    ...
    return {
        ...
        'down_sample': [True, True, False],    # len == len(down_channels) per model's assert
        'num_down_layers': 1,
        'num_mid_layers': 1,
        'num_up_layers': 1,
